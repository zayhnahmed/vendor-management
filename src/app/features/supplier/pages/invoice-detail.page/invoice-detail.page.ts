...
import { Invoice, InvoiceService } from '../../../buyer/services/invoice/invoice.service';
...
export class SupplierInvoiceDetailPage implements OnInit {
  ...
  load(): void {
    ...
    this.error.set(null);
    this.invoiceService.getById(this.invoiceId, 'vendor').subscribe({
  ...
  cancel(): void {
    ...
    this.invoiceService.cancel(this.invoiceId).subscribe({
      next: () => { this.actionLoading.set(false); this.actionSuccess.set('Invoice cancelled.'); this.load(); },
      error: (err) => { this.actionLoading.set(false); this.actionError.set(err.error?.message ?? 'Failed to cancel.'); },
    });
  }
