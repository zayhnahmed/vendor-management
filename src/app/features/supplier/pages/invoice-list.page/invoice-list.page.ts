...
import { Invoice, InvoiceService, InvoiceStatus } from '../../../buyer/services/invoice/invoice.service';
...
export class SupplierInvoiceListPage implements OnInit {
  ...
  filtered = computed(() => {
    ...
    if (s) data = data.filter((i) =>
      (i.buyerName ?? '').toLowerCase().includes(s) ||
  ...
  ngOnInit(): void {
    this.invoiceService.list('vendor').subscribe({
  ...
  createInvoice(): void { this.router.navigate(['/supplier/invoices/create']); }
  viewInvoice(id: string): void { this.router.navigate(['/supplier/invoices', id]); }
