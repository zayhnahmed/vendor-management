...
export class SupplierPoListPage implements OnInit {
    ...
    if (search) {
      data = data.filter(
        (p) =>
          ...
          (p.buyerOrgId ?? '').toLowerCase().includes(search),
      );
    }
    return data;
  });
  ...
  ngOnInit(): void {
    this.poService.list('vendor').subscribe({
      next: (res) => {
  ...
  viewPo(id: string): void {
    this.router.navigate(['/supplier/purchase-orders', id]);
  }
